"""
VERIFICACIONES AUTOMATICAS - BENCHMARK RECTANGULAR 3D SEMANA 1 MCOC
====================================================================
Verifica todas las condiciones de aceptacion del modelo rectangular
"""
...
def verificar_secciones(datos):
    print("\n=== VERIFICACION DE SECCIONES (VERSION RECTANGULAR) ===")
    ...
    # Verify all beam sections are rectangular 0.60x0.80
    for nombre, sec in [('T', sec_T), ('L89', sec_L89), ('L5', sec_L5)]:
        ...
        assert abs(sec['A'] - 0.48) < tolerancia, f"Error: A={sec['A']} en {nombre}"
        assert abs(sec['Iy'] - 0.0256) < tolerancia, f"Error: Iy={sec['Iy']} en {nombre}"
        assert abs(sec['Iz'] - 0.0144) < tolerancia, f"Error: Iz={sec['Iz']} en {nombre}"
    print("[OK] Todas las vigas: rectangular 0.60x0.80m, A=0.48, Iy=0.0256, Iz=0.0144")
    
    # Verify all beam sections are identical
    assert abs(sec_T['A'] - sec_L89['A']) < tolerancia
    assert abs(sec_T['A'] - sec_L5['A']) < tolerancia
    print("[OK] Todas las vigas tienen las mismas propiedades rectangulares")
    
    print("\n  Tabla de propiedades de secciones vigas (RECTANGULAR):")
    print(f"  {'Seccion':20s} {'Tipo':12s} {'A(m2)':8s} {'Iy(m4)':10s} {'Iz(m4)':10s} {'J(m4)':10s}  Elementos")
    print(f"  {'-'*90}")
    print(f"  {'RECT Interior 8.90':20s} {'rectangular':12s} {sec_T['A']:8.4f} {sec_T['Iy']:10.6f} {sec_T['Iz']:10.6f} {sec_T['J']:10.6f}  10 (B-E)")
    print(f"  {'RECT Borde 8.90':20s} {'rectangular':12s} {sec_L89['A']:8.4f} {sec_L89['Iy']:10.6f} {sec_L89['Iz']:10.6f} {sec_L89['J']:10.6f}  9,11 (A-D, C-F)")
    print(f"  {'RECT Borde 5.00':20s} {'rectangular':12s} {sec_L5['A']:8.4f} {sec_L5['Iy']:10.6f} {sec_L5['Iz']:10.6f} {sec_L5['J']:10.6f}  5,6,7,8 (A-B,B-C,D-E,E-F)")
...
def verificar_BE_empotrado_empotrado(resultados):
    print("\n=== VERIFICACION VIGA B-E EMPOTRADO-EMPOTRADO (RECTANGULAR) ===")
    ...
    M_50 = diag.get('M_OpenSees_50', None)
    if M_50 is not None:
...
def generar_reporte(resultados, ruta_reporte):
    with open(ruta_reporte, 'w', encoding='utf-8') as f:
        f.write("REPORTE DE VERIFICACION - BENCHMARK RECTANGULAR 3D SEMANA 1 MCOC\n")
        f.write("=" * 60 + "\n\n")
...
def main():
    print("=" * 60)
    print("VERIFICACIONES AUTOMATICAS - BENCHMARK RECTANGULAR 3D")
    print("=" * 60)
    ...
    ruta_reporte = Path(__file__).parent / 'resultados_rectangular' / 'reporte_verificacion.txt'
    ...
    try:
        ...
        print("\n" + "=" * 60)
        print("[OK] TODAS LAS VERIFICACIONES PASARON (RECTANGULAR)")
        print("=" * 60)
