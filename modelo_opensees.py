"""
MODELO OPENSEESPY - BENCHMARK 3D SEMANA 1 MCOC
================================================
...
def crear_secciones(E, G, datos):
    ...
    print("\n=== SECCIONES (colaboracion monolitica losa-viga) ===")
    print(f"  [1] Columna P70x70  : A={col['A']:.4f} m2, Iy={col['Iy']:.6f}, Iz={col['Iz']:.6f}, J={col['J']:.6f}")
    print(f"  [2] T_INTERIOR_8.90 : A={sec_T['A']:.4f} m2, Iy={sec_T['Iy']:.6f}, Iz={sec_T['Iz']:.6f}, J={sec_T['J']:.6f}")
    print(f"  [3] L_BORDE_8.90    : A={sec_L89['A']:.4f} m2, Iy={sec_L89['Iy']:.6f}, Iz={sec_L89['Iz']:.6f}, J={sec_L89['J']:.6f}")
    print(f"  [4] L_BORDE_5.00    : A={sec_L5['A']:.4f} m2, Iy={sec_L5['Iy']:.6f}, Iz={sec_L5['Iz']:.6f}, J={sec_L5['J']:.6f}")
...
def crear_elementos():
    ...
    print("  Vigas L 5.00m    (sec 4): 5(A-B), 6(B-C), 7(D-E), 8(E-F)")
    print("  Vigas L 8.90m    (sec 3): 9(A-D), 11(C-F)")
    print("  Viga  T 8.90m    (sec 2): 10(B-E)")
...
def aplicar_restricciones_RX():
    # B-E (elem 10) se comporta como empotrado-empotrado.
    # La flexion vertical ocurre en el plano Y-Z (local_x=Y, local_z=Z).
    # La rotacion relevante es RX (alrededor de X global = eje de flexion).
    # Se restringe solo RX en nodos 11 (B) y 14 (E).
    # DOF: 1=UX, 2=UY, 3=UZ, 4=RX, 5=RY, 6=RZ
    fix(11, 0, 0, 0, 1, 0, 0)  # RX_B = 0
    fix(14, 0, 0, 0, 1, 0, 0)  # RX_E = 0
...
def crear_cargas():
    ...
    # ------------------------------------------------------------------
    # Discretizacion triangular para vigas de 5m
    # Distribucion: 0 en extremos, 12.5 kN/m en centro (x=2.5m)
    # 4 intervalos iguales de 1.25 m, punto medio de cada intervalo
    # w(x) = 12.5 * (x/2.5) para x <= 2.5
    # ------------------------------------------------------------------
    L5 = 5.0
    n_reg5 = 4
    dx5 = L5 / n_reg5
    ...
    # Verificar primer momento respecto al extremo i (x=0)
    ...
    # Aplicar a vigas X (tags 5-8)
    for tag in [5, 6, 7, 8]:
        for xi, Pi in zip(xL_5, P_5):
            eleLoad('-ele', tag, '-type', '-beamPoint', 0.0, -Pi, xi, 0.0)

    # ------------------------------------------------------------------
    # Discretizacion trapezoidal para vigas de 8.9m
    # 3 regiones: rampa(2.5m), uniforme(3.9m), rampa(2.5m)
    # ------------------------------------------------------------------
    L89 = 8.9
    a_rampa = 2.5
    b_const = 6.4

    # Region 1: Rampa izquierda [0, 2.5m]
    n_reg1 = 4
    dx1 = a_rampa / n_reg1
    xL_r1 = []
    P_r1 = []
    ...
    # Region 3: Rampa derecha [6.4m, 8.9m]
    n_reg3 = 4
    ...
    # Verificar primer momento (simetrica -> resultante en L/2 = 4.45m)
    ...
    # Aplicar a vigas Y borde (tags 9, 11)
    for tag in [9, 11]:
        for xi, Pi in zip(xL_89, P_89):
            eleLoad('-ele', tag, '-type', '-beamPoint', 0.0, -Pi, xi, 0.0)

    # Aplicar a viga B-E interior (tag 10) con doble magnitud
    for xi, Pi in zip(xL_89, P_89):
...
def momento_interno(x, My_i, Fz_i, L, puntos_carga):
    """
    Momento flector en posicion x de una viga con fuerzas de extremo
    y cargas puntuales distribuidas (Metodo de los cortantes).

    M(x) = My_i + Fz_i * x - sum_{s_k < x} P_k * (x - s_k)

    Parametros
    ----------
    x : float
        Posicion a lo largo del eje local (m), 0 <= x <= L.
    My_i : float
        Momento flector en extremo i (kN*m), de eleForce local.
        0 para simplemente apoyada, !=0 para empotrado.
    Fz_i : float
        Fuerza vertical en extremo i (sistema local), positiva = hacia arriba.
    L : float
        Longitud del elemento (m).
    puntos_carga : list of dict
        Cada dict tiene 'x/L' (fraccion de longitud) y 'P' (magnitud, positiva = abajo).

    Retorna
    -------
    float : Momento flector en kN*m (convencion: positivo = traccion abajo / sagging).
    """
    M = My_i + Fz_i * x
...
def cortante_interno(x, Fz_i, L, puntos_carga):
    """
    Cortante vertical en posicion x de una viga.

    V(x) = Fz_i - sum_{s_k <= x} P_k

    Retorna V en kN (convencion local).
    """
    V = Fz_i
    for pc in puntos_carga:
        s = pc['x/L'] * L
...
def momento_interno_analitico_ss(x, L, w_max, a_rampa, b_const):
    """
    Momento flector exacto por viga SIMPLEMENTE APOYADA con carga
    trapezoidal (rampa-uniforme-rampa).

    M(x) = R*x - integral_0^x w(s)*(x-s) ds

    w(s):
      region 1 [0, a]:  rampa lineal 0 -> w_max
      region 2 [a, b]:  uniforme w_max
      region 3 [b, L]:  rampa lineal w_max -> 0
    """
    a = a_rampa
    b = b_const
    c = L - b
    ...
    if x <= a:
        # integral_0^x (w_max*s/a)*(x-s) ds = w_max*x^3/(6a)
        M -= (w_max / (6.0 * a)) * x ** 3
    elif x <= b:
        # Term 1 (x in upper limit): integral_0^a (w_max*s/a)*(x-s) ds
        M -= (w_max * a / 6.0) * (3.0 * x - 2.0 * a)
        # Term 2: integral_a^x w_max*(x-s) ds = w_max*(x-a)^2/2
        M -= w_max * (x - a) ** 2 / 2.0
    else:
        # Term 1 (complete region 1): integral_0^a (w_max*s/a)*(x-s) ds
        M -= (w_max * a / 6.0) * (3.0 * x - 2.0 * a)
        # Term 2 (complete region 2): integral_a^b w_max*(x-s) ds
        M -= w_max * (b - a) * (x - (a + b) / 2.0)
        # Term 3: integral_b^x w_max*(L-s)/(L-b)*(x-s) ds
        d = x - b
        M -= w_max * d ** 2 * (3.0 * c - d) / (6.0 * c)
    return M

def _w_trapezoidal(s, L, w_max, a_rampa, b_const):
    """Distribucion de carga trapezoidal w(s) punto a punto."""
    if s <= a_rampa:
...
def calcular_fijos_analitico(L, w_max, a_rampa, b_const, n_quad=1000):
    """
    Calcula momentos fijos de empotramiento y reacciones por integracion
    numerica (Simpson) de la carga trapezoidal sobre la viga.

    M_B = -1/L^2 * integral_0^L w(s)*s*(L-s)^2 ds
    M_E = -1/L^2 * integral_0^L w(s)*s^2*(L-s) ds   [signo: positivo a la derecha]
    R   = (integral_0^L w(s)*s ds) / L
...
def momento_ff_analitico(x, L, M_B, R, w_max, a_rampa, b_const):
    """
    Momento flector para viga EMPOTRADO-EMPOTRADO.
    M(x) = M_B + (M_ss(x) - R_ss*x) + R*x
    Como R_ff = R_ss, esto se reduce a M_B + M_ss(x).
    """
    return M_B + momento_interno_analitico_ss(x, L, w_max, a_rampa, b_const)

def calcular_diagrama_BE(info_cargas, resultados):
    """
    Calcula el momento flector M(x) a lo largo de la viga B-E (elem 10)
    usando fuerzas de extremo + cargas puntuales eleLoad (exacto para
    elemento Hermitiano con forma cubica).

    Incluye comparacion con soluciones simplemente apoyada y empotrado-empotrado.
    """
    L_BE = 8.9
    n_pts = 100
    ...
    # Momento numerico (fuerzas de extremo + discretizado)
    ...
    # Simplemente apoyada (analitica exacta)
    M_ss = np.array([
        momento_interno_analitico_ss(xL * L_BE, L_BE, 25.0, 2.5, 6.4)
        for xL in xL_vals
    ])
    # Empotrado-empotrado (FEM analitico por integracion numerica de Simpson)
    ...
    print("\n=== MOMENTO INTERNO VIGA B-E (elem 10) ===")
...
def calcular_diagramas_vigas(info_cargas, resultados):
    """
    Calcula diagramas de cortante V(x) y momento M(x) para las 7 vigas.
    Retorna dict con datos de diagramas para graficacion.
    """
    n_pts = 100
    xL_vals = np.linspace(0.0, 1.0, n_pts + 1)
    ...
    diagramas = {}
    print("\n=== DIAGRAMAS DE FUERZAS INTERNAS (7 VIGAS) ===")
...
def verificar_ejes_locales():
    ...
    # Verificar que gravedad (-Z global) corresponde a -local_z para vigas
...
def extraer_resultados(ejes_locales):
    ...
    elem_info = {
        ...
        9: 'Vig A-D L89', 10: 'Vig B-E T', 11: 'Vig C-F L89'
    }
    ...
    # Fuerzas de elementos: globales y locales
    print("\n=== FUERZAS DE ELEMENTOS ===")

    for tag in range(1, 12):
        f_global = np.array(eleForce(tag))

        # Obtener ejes locales y construir matriz de rotacion
        xax = np.array(eleResponse(tag, 'xaxis'))
        yax = np.array(eleResponse(tag, 'yaxis'))
        zax = np.array(eleResponse(tag, 'zaxis'))
        R = np.array([xax, yax, zax])

        # Transformar a locales: F_local = R * F_global
        Fg_i = f_global[0:3]
        Mg_i = f_global[3:6]
        Fg_j = f_global[6:9]
        Mg_j = f_global[9:12]
        ...
        # Guardar fuerzas globales (etiquetas correctas)
...
def imprimir_diagnostico(resultados, info_cargas, diagrama_BE):
    print("\n" + "=" * 65)
    print("  DIAGNOSTICO DE VERIFICACION")
    print("=" * 65)

    # Columna A: verificacion axial
    ...
    # Viga B-E: verificacion de esfuerzos internos (empotrado-empotrado)
    fl_BE = resultados['fuerzas_locales_elementos']['elem_10']
    fg_BE = resultados['fuerzas_globales_elementos']['elem_10']
    print(f"\n  VIGA B-E (elem 10) - EMPOTRADO-EMPOTRADO:")
    ...
    # Verificar cortantes = carga total
...
def guardar_resultados(resultados, info_cargas, diagrama_BE, diagramas_vigas):
    ...
    ruta = Path(__file__).parent / 'resultados' / 'resultados.json'
...
def main():
    ...
    print("=" * 65)
    print("  MODELO OPENSEESPY - BENCHMARK 3D SEMANA 1 MCOC")
    print("=" * 65)
